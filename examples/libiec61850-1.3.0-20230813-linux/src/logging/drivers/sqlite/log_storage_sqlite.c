...
{
    printf("SqliteLogStorage_createInstance(const char* filename = %s)\n", filename);
    sqlite3* db = NULL;
    ...
    printf("sqlite3_open(filename = %s, &db) = rc = %d\n", filename, rc);
        ...
        LogEntryCallback entryCallback, LogEntryDataCallback entryDataCallback, void* parameter)
{
    ...
    while ((rc = sqlite3_step(instanceData->getEntriesAfter)) == SQLITE_ROW) {
        ...
        //else
        //{
            //printf("log get with time limit (startingTime = %lld >= timestamp = %lld) not matched\n", startingTime, timestamp);
        //}
    }
    if (sendFinalEvent)
        if (entryCallback != NULL)
