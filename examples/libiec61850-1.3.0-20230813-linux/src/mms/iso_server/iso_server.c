    ...
    for (i = 0; i < CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS; i++) {
        if (self->openClientConnections[i] != NULL) {
            if (IsoConnection_isRunning(self->openClientConnections[i])) {
				...
				printf("handleClientConnections: IsoConnection_destroy[%i].\n", i);
                IsoConnection_destroy(self->openClientConnections[i]);
...
static IsoConnection
handleIsoConnections(IsoServer self)
{
	IsoConnection oBackObj = NULL;
    ...
    if ((connectionSocket = ServerSocket_accept((ServerSocket) self->serverSocket)) != NULL) {
        ...
        if (isoConnection) {
			...
			//output connection
			oBackObj = isoConnection;
			printf("add new Connection clientAddr=%s\n", IsoConnection_getPeerAddress(isoConnection));
        }
...
handleIsoConnectionsThreadless(IsoServer self)
{
    ...
    if ((connectionSocket = ServerSocket_accept((ServerSocket) self->serverSocket)) != NULL) {
        ...
        if (self->maxConnections > -1) {
            if (private_IsoServer_getConnectionCounter(self) >= self->maxConnections) {
                Socket_destroy(connectionSocket);
				printf("ISO_SERVER: maximum number[%i] of connections reached -> reject connection attempt.\n", self->maxConnections);
                return;
            }
        }
#endif /* (CONFIG_MMS_SERVER_CONFIG_SERVICES_AT_RUNTIME == 1) */
        ...
        if (private_IsoServer_getConnectionCounter(self) >= CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS) {
            Socket_destroy(connectionSocket);
            handleClientConnections(self);
			printf("ISO_SERVER: maximum number CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS[%i] of connections reached -> reject connection attempt.\n", CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS);
            return;
        }
#endif
...
isoServerThread(void* isoServerParam)
{
	...
	uint64_t uTimeRecord = Hal_getTimeInMs();
    if (DEBUG_ISO_SERVER)
        printf("ISO_SERVER: isoServerThread %p started\n", &isoServerParam);

    while (self->state == ISO_SVR_STATE_RUNNING)
    {
		IsoConnection oNewConnectObj = handleIsoConnections(self);
		//connect check
		//remove sameIP
		if(oNewConnectObj != NULL)
		{
			char *pNewIP = IsoConnection_getPeerAddress(oNewConnectObj);
			char *p1 = pNewIP;
			int nIpSize = 0;
			while (p1[nIpSize] != ':' && p1[nIpSize] != 0)
			{
				nIpSize++;
			}
			//int nIpSize = sizeof(pNewIP);
			int nHaveMoveCount = 0;
			int m = 0;
			for (; m < CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS; m++) {
				if (self->openClientConnections[m] != NULL) {
					if (oNewConnectObj != self->openClientConnections[m])
					{
						char *pOldIP = IsoConnection_getPeerAddress(self->openClientConnections[m]);

						char *p2 = pOldIP;
						int nIpSizeOld = 0;
						while (p2[nIpSizeOld] != ':' && p2[nIpSizeOld] != 0)
						{
							nIpSizeOld++;
						}

						if (nIpSizeOld == nIpSize)
						{
							if (memcmp(pOldIP, pNewIP, nIpSize) == 0)
							{
								printf("reconnect ip(%s)[size:%i] have removed;\n", pOldIP, nIpSize);
								//IsoConnection_close(self->openClientConnections[m]);
								IsoConnection_stop(self->openClientConnections[m]);
								nHaveMoveCount++;
								//IsoConnection_destroy(self->openClientConnections[m]);
							}
						}
			...
			{
				int m = 0;
				uint64_t nMinestIndex = -1;
				int nSelectIndex = -1;
				for (; m < CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS; m++) {
					if (self->openClientConnections[m] != NULL) {
						uint64_t nCreateTimeInfo = getCreateTime(self->openClientConnections[m]);
						if (nMinestIndex == -1)
						{
							nMinestIndex = nCreateTimeInfo;
							nSelectIndex = m;
						}
						else
						{
							if (nMinestIndex > nCreateTimeInfo)
							{
								nMinestIndex = nCreateTimeInfo;
								nSelectIndex = m;
							}
				...
				if (nSelectIndex != -1)
				{
					printf("more connections, ip(%s) have removed;\n", IsoConnection_getPeerAddress(self->openClientConnections[nSelectIndex]));
					//IsoConnection_close(self->openClientConnections[nSelectIndex]);
					IsoConnection_stop(self->openClientConnections[nSelectIndex]);
					nHaveMoveCount++;
					//IsoConnection_destroy(self->openClientConnections[nSelectIndex]);
				}
			}

			//output all connect
			int nCurrentConnectCount = private_IsoServer_getConnectionCounter(self);
			nCurrentConnectCount = 0;
			//遍历一下
			int n = 0;
			for (; n < CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS; n++) {
				if (self->openClientConnections[n] != NULL) {
					if (IsoConnection_isRunning(self->openClientConnections[n]))
					{
						nCurrentConnectCount++;
					}
				}
			}
			int nAllConnectCount = self->maxConnections - 1;
			printf("current connections count(%i), the maxcount is[%i], can continue connect[%i];\n", nCurrentConnectCount, nAllConnectCount, nAllConnectCount - nCurrentConnectCount);
		}
		
		//每间隔2秒就会触发一次打印当前全部的连接对象信息
		uint64_t uTimeCurrent = Hal_getTimeInMs();
		if (uTimeCurrent - uTimeRecord > 4000)
		{
			uTimeRecord = uTimeCurrent;
			printf("\n\n");
			printf("current connections:");
			//输出全部的连接信息
			int k = 0;
			int nConnectionCount = 0;
			for (; k < CONFIG_MAXIMUM_TCP_CLIENT_CONNECTIONS; k++) {
				if (self->openClientConnections[k] != NULL) {
					if (IsoConnection_isRunning(self->openClientConnections[k]))
					{
						printf("connection IP:%s \n", IsoConnection_getPeerAddress(self->openClientConnections[k]));
						nConnectionCount++;
					}
				}
			}
			printf("total connection count: %i\n", nConnectionCount);
			printf("\n\n\n");
		}
