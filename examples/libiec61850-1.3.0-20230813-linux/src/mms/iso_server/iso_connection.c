...
struct sIsoConnection
{
	...
	uint64_t nCreateTimeinfo;//first create time
};
...
IsoConnection_create(Socket socket, IsoServer isoServer)
{
	...
	self->nCreateTimeinfo = Hal_getTimeInMs();
...
IsoConnection_sendMessage(IsoConnection self, ByteBuffer* message, bool handlerMode)
{
	...
	if (indication != COTP_OK)
	{
		printf("ISO_SERVER: IsoConnection_sendMessage failed~ stop connection!\n");
...
uint64_t getCreateTime(IsoConnection self)
{
	return self->nCreateTimeinfo;
}
...
void
IsoConnection_stop(IsoConnection self)
{
	if (self->state != ISO_CON_STATE_STOPPED) {
		self->state = ISO_CON_STATE_STOPPED;
	}
