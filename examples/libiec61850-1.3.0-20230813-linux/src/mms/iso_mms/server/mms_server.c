...
MmsServer_create(MmsDevice* device, TLSConfiguration tlsConfiguration)
{
    ...
    //userlock 和 model分开
    self->modelMutex_user = Semaphore_create(1);
    IsoServer_setUserLock(self->isoServer, self->modelMutex_user);
...
MmsServer_destroy(MmsServer self)
{
    ...
    Semaphore_destroy(self->modelMutex_user);
#endif
