...
addFileEntriesToResponse(const char* basepath, uint8_t* buffer, int bufPos, int maxBufSize, char* directoryName, char** continueAfterFileName, bool* moreFollows)
{
    ...
    printf("addFileEntriesToResponse directoryName = %s\n", directoryName);
    ...
    if (directory != NULL) {
        ...
        printf("FileSystem_readDirectory directory filename= %s\n", fileName);

        while (fileName != NULL) {

            if (isDirectory && strlen(fileName) != 0)
			{
				//文件夹
				uint64_t msTime;
				...
				//添加 /
				char fileNameClone[129];
				fileNameClone[0] = '/';
				memcpy(fileNameClone + 1, fileName, strlen(fileName));
				fileNameClone[strlen(fileName) + 1] = 0;
				fileNameClone[strlen(fileName) + 2] = 0;

				if (getFileInfo(basepath, fileNameClone, &fileSize, &msTime)) {
					char gtString[30];
					...
					int filenameSize = encodeFileSpecification(0xa0, fileNameClone, NULL, 0);
					...
					if (overallEntrySize > bufferSpaceLeft) {
						*moreFollows = true;
						...
						bufPos = encodeFileSpecification(0xa0, fileNameClone, buffer, bufPos); /* fileName */
            ...
            printf("addFileEntriesToResponse directory filename= %s\n", fileName);
            if (*moreFollows == true)
                break;
        ...
        printf("openDirectory directory is null\n");
    ...
    ByteBuffer* response)
{
    ...
    printf("**************FileDirectory = %s\n", MmsServerConnection_getFilesystemBasepath(connection));
