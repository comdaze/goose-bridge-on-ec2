...
{
    if (DEBUG_IED_SERVER)
        printf("IED_SERVER: reporting.c: run purgeBuf\n");
...
void ReportControl_destroy(ReportControl *self)
{
...
sendReport(ReportControl *self, bool isIntegrity, bool isGI)
{
    ...
    // MmsValue_setBitStringBit(optFlds, 7, true); /* entryID */
    ...
    if (MmsValue_getBitStringBit(optFlds, 6))
    { /* bufOvfl */
    ...
    MmsServerConnection_sendInformationReportVMDSpecific(self->clientConnection, "RPT", reportElements, true);
            ...
            if (dataSet == NULL)
            {
                ...
                if (dataSetName[0] == '@')
                {
                    ...
                    { /* for buffered report non-permanent datasets are not allowed */
                        if (connection != NULL)
                        {
                            MmsNamedVariableList mmsVariableList = MmsServerConnection_getNamedVariableList(connection, dataSetName + 1);
        ...
        if (owner != NULL)
        {
            ...
            if (connection != NULL)
            {
                ...
                else
                {
                    ...
                    if (DEBUG_IED_SERVER)
                        printf("IED_SERVER: reporting.c:   client address is IPv6 address or unknown\n");
                }
            }
            else
            {
    ...
    if (strcmp(elementName, "RptEna") == 0)
    {
        ...
        if (value->value.boolean == true)
        {
            ...
            if (updateReportDataset(self, rc, NULL, connection))
            {
                ...
                printf("Report RCB name: %s:set Enable = true\n",
                       rc->name);
                rc->enabled = true;
                rc->gi = false;
        ...
        else
        {
            ...
            printf("Report RCB name: %s:set Enable = false\n",
                   rc->name);
            rc->enabled = false;
        }
    }
...
void Reporting_deactivateReportsForConnection(MmsMapping *self, MmsServerConnection connection)
{
...
#define LOCAL_STORAGE_MEMORY_SIZE 65536
    ...
    // printf("Send Report IMINFO: RCB name: %s (SQN:%u) enabled:%i buffered:%i buffering:%i intg:%i\n",
    //        self->name, (unsigned)self->sqNum, self->enabled,
    //        self->isBuffering, self->buffered, self->intgPd);
    ...
    if (MmsValue_getBitStringBit(optFlds, 2))
    { /* report time stamp */
    ...
    if (MmsValue_getBitStringBit(optFlds, 4))
    { /* data set reference */
        MmsValue *datSet = ReportControl_getRCBValue(self, "DatSet");
    ...
    {
        printf("if (MmsValue_getBitStringBit(optFlds, 7)) ---no entryID info\n");
    }
    ...
    { /* data-reference */
        ...
        for (i = 0; i < self->dataSet->elementCount; i++)
        {
            ...
            if (addReferenceForEntry)
            {
                ...
                int refLen = iedNameLength + ldNameLength + variableNameLength + 1;
    ...
    if (MmsValue_getBitStringBit(optFlds, 3))
    {
        ...
        for (i = 0; i < self->dataSet->elementCount; i++)
        {
            ...
            else if (MmsValue_getBitStringBit(inclusionField, i))
            {
                ...
                int reasonTag = (int)*currentReportBufferPos;
                if(reasonTag & REPORT_CONTROL_QUALITY_CHANGED)
                {
                    MmsValue_setBitStringBit(reason, 2, true);
                }
                if(reasonTag & REPORT_CONTROL_VALUE_CHANGED)
                {
                    MmsValue_setBitStringBit(reason, 1, true);
                }
                if(reasonTag & REPORT_CONTROL_VALUE_UPDATE)
                {
                    MmsValue_setBitStringBit(reason, 3, true);
                }
    ...
    // printf("sendNextReportEntry: MmsServerConnection_sendInformationReportVMDSpecific!\n");
    MmsServerConnection_sendInformationReportVMDSpecific(self->clientConnection, "RPT", (LinkedList)reportElements, true);
...
void Reporting_activateBufferedReports(MmsMapping *self)
{
...
void Reporting_processReportEvents(MmsMapping *self, uint64_t currentTimeInMs)
{
...
void Reporting_processReportEventsAfterUnlock(MmsMapping *self)
{
    LinkedList element = self->reportControls;
...
void ReportControl_valueUpdated(ReportControl *self, int dataSetEntryIndex, int flag, bool modelLocked)
{
    ReportControl_lockNotify(self);
	
	//如果报告事假已存在，可以合并品质变化和数据变化品质位
    uint8_t oldFlag_dt = self->inclusionFlags[dataSetEntryIndex] & REPORT_CONTROL_VALUE_CHANGED;
    uint8_t oldFlag_q = self->inclusionFlags[dataSetEntryIndex] & REPORT_CONTROL_QUALITY_CHANGED;

    uint8_t newFlag_dt = flag & REPORT_CONTROL_VALUE_CHANGED;
    uint8_t newFlag_q = flag & REPORT_CONTROL_QUALITY_CHANGED;
    if (oldFlag_dt != 0 && oldFlag_q == 0 && newFlag_q != 0 && newFlag_dt == 0)
    {
        self->inclusionFlags[dataSetEntryIndex] |= flag;
    }
    else if (oldFlag_dt == 0 && oldFlag_q != 0 && newFlag_q == 0 && newFlag_dt != 0)
    {
        self->inclusionFlags[dataSetEntryIndex] |= flag;
    }
    else
    {
        if (self->inclusionFlags[dataSetEntryIndex] != 0)
        { /* report for this data set entry is already pending (bypass BufTm) */
