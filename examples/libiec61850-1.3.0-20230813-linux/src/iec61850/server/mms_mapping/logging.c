...
{
    printf("LogInstance_create(LogicalNode* parentLN, const char* name = %s)\n", name);
...
{
    printf("enableLogging(LogControl* self)\n");
    ...
    else
    {
        ...
        {
            printf("self->dataSet == NULL\n");
        }

        if(self->logInstance == NULL)
        {
            printf("self->logInstance == NULL\n");
        }
        return false;
    }
    return false;      
...
{
    printf("getLogInstanceByLogRef(logRef = %s)\n", logRef);
    char refStr[130];
    char* domainName;
    char* lnName;
    char* logName;
    ...
    printf("domainName = %s, lnName = %s, logName = %s\n", domainName, lnName, logName);

    if(self->logInstances == NULL)
    {
        printf("self->logInstances == NULL\n");
    }
    else
    {
        printf("self->logInstances != NULL\n");
    ...
    printf("LinkedList instance = LinkedList_getNext(self->logInstances);\n");

    if(instance == NULL)
    {
        printf("if(instance == NULL)**********************\n");
    }

    while (instance != NULL) {
        printf("while (instance != NULL) \n");
        LogInstance* logInstance = (LogInstance*) LinkedList_getData(instance);
        printf("logInstance->name = %s, logName = %s \n", logInstance->name, logName);
        if (strcmp(logInstance->name, logName) == 0) {
            printf("strcmp(logInstance->name, logName) == 0 \n");
            printf("lnName = %s logInstance->parentLN->name = %s \n", lnName, logInstance->parentLN->name);
            if (strcmp(lnName, logInstance->parentLN->name) == 0) {
                printf("strcmp(lnName, logInstance->parentLN->name) == 0) \n");
                LogicalDevice* ld = (LogicalDevice*) logInstance->parentLN->parent;

                printf("ld->name = %s domainName = %s \n", ld->name, domainName);
                ...
                do
                {
                    ...
                    ldback--;
                    domainNameBack--;
                }while(ldback != ld->name && domainNameBack != domainName);

                if(bMacthTag && ldback == ld->name)
                {
                    printf("strcmp(ld->name, domainName) == 0 \n");
                    return logInstance;
                }
    ...
    printf("getLogInstanceByLogRef finish\n");
...
{
    printf("LIBIEC61850_LOG_SVC_writeAccessLogControlBlock\n");
    ...
    *separator = 0;
    ...
    if (strcmp(varName, "LogEna") == 0) {
        printf("strcmp(varName, LogEna) == 0\n");
        ...
        if (logEna == false) {
            ...
            if (enableLogging(logControl)) {
                ...
                printf("enableLogging(logControl) success\n");
                if (DEBUG_IED_SERVER)
                    printf("IED_SERVER: enabled log control %s\n", logControl->name);
            }
            else
            {
                printf("enableLogging(logControl) error\n");
        ...
        if (logControl->enabled == false) {
            ...
            if (logRef == NULL) {
               ...
               if (strcmp(logRef, "") == 0) {
                   ...
                   printf("LogInstance* logInstance = getLogInstanceByLogRef(self, logRef)\n");
    ...
    if (updateValue) {
        ...
        if(NULL == element)
        {
            printf("updateValue[NULL == element]===>varName = %s\n", varName);
        }
        else
        {
        ...
        int lcbCount)
{
    ...
    while (currentLcb < lcbCount) {
        ...
        if (logControlBlock->logRef != NULL)
        {
            ...
            printf("logControl->logInstance = getLogInstanceByLogRef(self, logControlBlock->logRef)\n");
        }
...
{
    printf("MmsMapping_setLogStorage(logRef = %s)\n", logRef);
    LogInstance* logInstance = getLogInstanceByLogRef(self, logRef);

    printf("LogInstance* logInstance = getLogInstanceByLogRef(self, logRef);\n");

    if (logInstance != NULL) {
        printf("logInstance != NULL\n");
        LogInstance_setLogStorage(logInstance, logStorage);

        printf("LogInstance_setLogStorage(logInstance, logStorage);\n");

        char domainName[65];

        MmsMapping_getMmsDomainFromObjectReference(logRef, domainName);

        printf("MmsMapping_getMmsDomainFromObjectReference(logRef = %s, domainName = %s);\n", logRef, domainName);
