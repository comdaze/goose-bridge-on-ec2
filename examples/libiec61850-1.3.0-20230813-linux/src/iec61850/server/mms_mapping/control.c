        ...
        MmsValue* value, MmsServerConnection connection)
{
    ...
    *separator = 0;
    ...
    if (strcmp(varName, "SBOw") == 0) { /* select with value */
            ...
            if (ctlVal != NULL) {
                ...
                if (checkValidityOfOriginParameter(origin) == false) {
                    ...
                    ControlObject_sendLastApplError(controlObject, connection, "SBOw", 0,
                                ADD_CAUSE_SELECT_FAILED, ctlNum, origin, true);//额外添加错误返回值
                    goto free_and_return;
                }

                bool testCheck = MmsValue_getBoolean(test);//不支持true操作
                if (testCheck == true) {
                    printf("select with value test is true cause ADD_CAUSE_BLOCKED_BY_MODE\n");
                    indication = DATA_ACCESS_ERROR_OBJECT_ATTRIBUTE_INCONSISTENT;
                    ControlObject_sendLastApplError(controlObject, connection, "SBOw", 0,
                                ADD_CAUSE_BLOCKED_BY_MODE, ctlNum, origin, true);//额外添加错误返回值
                    goto free_and_return;
                }
                ...
                if (state != STATE_UNSELECTED) {
                    ...
                    {
                        //ControlObject_sendLastApplError(controlObject, connection, "SBOw", 0,
                                //ADD_CAUSE_COMMAND_ALREADY_IN_EXECUTION, ctlNum, origin, true);//第一版返回规则
                        ControlObject_sendLastApplError(controlObject, connection, "SBOw", 0,
                                ADD_CAUSE_LOCKED_BY_OTHER_CLIENT, ctlNum, origin, true);//第二版返回规则
                    }  
                    else
                        ControlObject_sendLastApplError(controlObject, connection, "SBOw", 0,
        ...
        bool testCheck = MmsValue_getBoolean(test);//不支持true操作
        if (testCheck == true) {
            printf("operate test is true cause ADD_CAUSE_BLOCKED_BY_MODE\n");
            indication = DATA_ACCESS_ERROR_OBJECT_ATTRIBUTE_INCONSISTENT;
             ControlObject_sendLastApplError(controlObject, connection, "Oper",
                    0, ADD_CAUSE_BLOCKED_BY_MODE,
                        ctlNum, origin, true);
            goto free_and_return;
        }
        ...
        if (state == STATE_WAIT_FOR_ACTIVATION_TIME) {
            indication = DATA_ACCESS_ERROR_TEMPORARILY_UNAVAILABLE;
            printf("if (state == STATE_WAIT_FOR_ACTIVATION_TIME)\n");
            ControlObject_sendLastApplError(controlObject, connection, "Oper",
            ...
            if (operTm != NULL && false) {//禁止激活时间条件，确保激活无效
                ...
                if (controlObject->operateTime != 0) {
                    ...
                    printf("setState(controlObject, STATE_WAIT_FOR_ACTIVATION_TIME)\n");
