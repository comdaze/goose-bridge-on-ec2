    ...
    if (self->iedServer->logServiceEnabled) {
        ...
        while (log != NULL) {
            ...
            if (nameLength > 63) {
                ...
                if(logInstance != NULL)
                {
                    printf("LogInstance_create(log->parent, log->name = %s) success\n", log->name);
                }
                else
                {
                    printf("LogInstance_create(log->parent, log->name = %s) error\n", log->name);
                }
        ...
        char* variableId, MmsValue* value, MmsServerConnection connection)
{
	...
	char bufferSet[200];
	bufferSet[0] = 0;
	MmsValue_printToBuffer(value, bufferSet, 200);
	printf("DO IED_SERVER_WRITE: %s.%s = %s\n", domain->domainName, variableId, bufferSet);
                        ...
                        if (sg->sgcb->editSG != 0) {
                            if (sg->editingClient == (ClientConnection) connection) {
                                if (sg->editSgConfirmedHandler != NULL) {
									if (sg->editSgConfirmedHandler(sg->editSgConfirmedHandlerParameter, sg->sgcb,
										sg->sgcb->editSG))
									{
										unselectEditSettingGroup(sg);
										return DATA_ACCESS_ERROR_SUCCESS;
...
MmsMapping_triggerLogging(MmsMapping* self, MmsValue* value, LogInclusionFlag flag)
{
    ...
    while ((element = LinkedList_getNext(element)) != NULL) {
        ...
        if ((lc->enabled) && (lc->dataSet != NULL)) {
            ...
            dataRef[0] = 0;
            dataRef[1] = 0;
            printf("DataSet_isMemberValueWithRef(lc->dataSetRef = %s, value, self->model->name = %s)\n", lc->dataSetRef, self->model->name);
            if (DataSet_isMemberValueWithRef(lc->dataSet, value, dataRef, self->model->name)) 
            //if(memcmp(lc->dataSetRef, cmpDt, 18) == 0 && lc->enabled == true)
            {
                printf("make log info dataRef = %s\n", dataRef);
            
                if (lc->logInstance != NULL) {
                    printf("LogInstance_logSingleData(lc->logInstance, dataRef, value, reasonCode);\n");
            ...
            {
                printf("DataSet_isMemberValueWithRef(lc->dataSetRef, value, dataRef, self->model->name) = error)\n");
            }

            printf("DataSet_isMemberValueWithRef(lc->dataSetRef = %s, value, dataRef = %s, self->model->name = %s)\n", lc->dataSetRef, dataRef, self->model->name);
        }
    }
}
