...
singleThreadedServerThread(void* parameter)
{
    ...
    printf("windows IED_SERVER: server thread started!\n");
...
{
    printf("IedServer_setLogStorage(IedServer self, const char* logRef = %s, LogStorage logStorage), CONFIG_IEC61850_LOG_SERVICE = %d\n", logRef, CONFIG_IEC61850_LOG_SERVICE);
#if (CONFIG_IEC61850_LOG_SERVICE == 1)
    printf("self->mmsMapping  check \n");
    if(self->mmsMapping == NULL)
    {
        printf("self->mmsMapping == NULL\n");
    }
    else
    {
        printf("self->mmsMapping != NULL\n");
